...
def upload():
    ...
    logging.info("The Value is %s", res)
    # 拡張子取得
    path, extention = os.path.splitext(file.filename)

    # front側で表示できる形にする
    src = "data:image/" + extention + ";base64," + res 
    app_data.set("src", src)
    app_data.dump()
...
def signup(user_id, password, account_picture, user_name):
    ...
    # 新規登録
    sql = "INSERT INTO account VALUES('" + user_id + "', '" + password + "', '" + app_data.get("src")+ "', '" + user_name + "')" # SQLコマンド
...
@app.route("/account_info/<user_id>")
def account_info_2(user_id):
...
def post_content():
    ...
    for row in data.execute("SELECT * FROM post ORDER BY post_id DESC"):
        # 投稿id, 投稿文章, 投稿画像, 投稿したユーザーの名前
...
def take_post(post_id):
    ...
    for row in data.execute(sql):
        ...
        # account(user_id text, password text, account_picture text, account_name text)
        for row_2 in data.execute("SELECT * FROM account WHERE account_name = '" + row[3] + "'"):
            post_data["account_picture"] = row_2[2]
...
# プロフィール写真変更
@app.route("/change_profile_picture/<profile_picture>", methods=["POST", "GET"])
def change_profile_picture(profile_picture):
    # ログインしているアカウント情報取得
    user_id = app_data.get("login_user")
    account = account_info(user_id)
    picture = app_data.get("src")

    # プロフィール写情報更新 account(user_id text, password text, account_picture text, account_name text)
    data = sqlite3.connect('DATA') #データベースに接続
    sql = "UPDATE account set account_picture = '" + picture + "' WHERE user_id = '" + account["user_id"] + "'"
    data.execute(sql)
    data.commit()
    data.close() #データベースを閉じる
    ...
    return jsonify({"result" : "success", "profile_pictue":profile_picture})
